/*无向无权图*/
#ifndef UNDRIECTEDUNWEIGHTEDGRAPH_H
#define UNDRIECTEDUNWEIGHTEDGRAPH_H
#include "../../utils/common.hpp"
using namespace std;
template<typename T>
class UnDriectedUnWeightedGraph
{
private:
public:
    ...
    /*构造方法*/
    UnDriectedUnWeightedGraph(const vector<vector<Vertex*>>&edges){
        //添加所有顶点和边
    ...
    vector<Vertex*>graphBFS(UnDriectedUnWeightedGraph&graph,Vertex*strat){
    ...
    /*无向图的深度优先遍历,递归*/
    void dfs(UnDriectedUnWeightedGraph&graph,unordered_set<Vertex*>&visited,vector<Vertex*>&res,Vertex*vet){
    ...
    /*辅助函数，传入参数*/
    vector<Vertex*>graphDFS(UnDriectedUnWeightedGraph&graph,Vertex*start){
        vector<Vertex*>res;
